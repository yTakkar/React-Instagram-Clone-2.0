...
export default class Emojis extends React.Component {
  ...
  selectEmoji = () => {
    $('.emoji_all li').on('click', e => {
      let
        { textArea, setState } = this.props,
        value = textArea.val(),
        { innerText: text } = e.target,
        start = textArea.prop('selectionStart'),
        textBefore = value.substr(0, start),
        textAfter = value.substr(start, value.length),
        finalText = textBefore+text+textAfter

      $('.emoji_all li').removeClass('emoji_toggle')
