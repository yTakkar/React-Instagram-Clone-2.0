...
app.post('/create-new-conversation', async (req, res) => {
  ...
  try {
    ...
    if(conExists == 0) {
      ...
      let { insertId } = await db.query('INSERT INTO conversations SET ?', conversation)
...
app.post('/get-conversation-details', async (req, res) => {
  let
    { id } = req.session,
    { con_id } = req.body,
    [ _con ] = await db.query('SELECT * FROM conversations WHERE con_id=?', [con_id])

  let
    con_with = _con.user_one == id ? _con.user_two : _con.user_one,
    con_with_username = await User.getWhat('username', con_with),
    con_with_firstname = await User.getWhat('firstname', con_with),
    con_with_surname = await User.getWhat('surname', con_with),
...
app.post('/unsend-all-mssgs', async (req, res) => {
  try {
    ...
    await db.query(
      'DELETE FROM messages WHERE con_id=? AND mssg_by=?',
      [ con_id, id ]
    )
