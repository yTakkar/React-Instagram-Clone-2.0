...
const commentDispatchHelper = async options => {
  let {
    user, post_id, comment_id, commentExtraDetails, when, dispatch
  } = options
...
export const imageComment = async options => {
  let
    {
      post_id, dispatch, when, user, file: commentFile, done
    } = options,
    form = new FormData(),
  ...
  if (success) {
    await commentDispatchHelper({
      user, post_id, comment_id, when, dispatch,
      commentExtraDetails: {
        type: 'image',
        commentSrc: filename,
        text: ''
...
export const textComment = async options => {
  let { post_id, text, when, dispatch, postOwner, done } = options

  let {
    data: { success, mssg, comment_id }
  } = await post('/api/comment-text', { post: post_id, text })

  if (success) {
    await commentDispatchHelper({
      user: postOwner,
      post_id, comment_id, when, dispatch,
      commentExtraDetails: {
        type: 'text',
        commentSrc: '',
        text
